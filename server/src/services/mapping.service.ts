...
const logger = new Logger('MappingService');

// ClickHouse data type mapping
const clickhouseTypeMapping: Record<string, string> = {
  // Common data types
  'String': 'String',
  'Int8': 'Int8',
  'Int16': 'Int16',
...
const inferClickHouseType = (value: any): string => {
...
class MappingService {
  inferSchemaFromData(data: any[]): { name: string, type: string }[] {
    if (!data.length) {
      return [];
    }
  ...
  mapDataToSelectedColumns(data: any[], selectedColumns: string[]): any[] {
    return data.map(row => {
